...
import { urlImg } from "src/webServices";
...
function Payment() {
  ...
  const [inputOTP, setInputOTP] = useState("formCard");
  ...
  const handleSubmit = (e) => {
    ...
    if (inputOTP === "formOTP") {
      if (OTPCard.trim() === "123456") {
        ...
        setInputOTP("formSuccess");
      } else {
        alert("Sai!");
      }
    }
    if (inputOTP === "formCard") {
      if (
        (numberCard.trim() === "9704250000000001") &
        (dateCard.trim() === "01/13") &
        (nameCard.trim() === "NGUYEN VAN A")
      ) {
        setInputOTP("formOTP");
      } else {
        alert("Sai!");
  ...
  return (
    <div className={clsx(styles["container"])}>
      ...
      {inputOTP === "formCard" || inputOTP === "formOTP" ? (
        <main
          className={clsx(styles["main"], styles["grid-main"], "container")}
        >
          ...
          <div className={clsx(styles["show-box"])}>
            ...
            <div className={clsx(styles["form-group"])}>
              ...
              >
                ...
                <p
                  className={clsx(styles["text-money"], "text-primary fw-bold")}
                >
                  {money} VND
                </p>
              </div>
        ...
        <main className={clsx(styles["main"], "container")}>
          <div className="mt-3">
            <p className="text-center text-large text-success fw-bold fs-1">
              Thanh toán thành công!
            </p>
            <div className="d-flex justify-content-center">
              <Link to="/" className={clsx(styles["link-close"], "p-3")}>
                Trở về trang chủ
              </Link>
            </div>
          </div>
        ...
        <main className={clsx(styles["main"], "container")}>
          <div className="mt-3">
            <p
              className={clsx(
                styles[""],
                "text-center text-large text-danger fw-bold fs-1"
              )}
            >
              Có lỗi!
            </p>
